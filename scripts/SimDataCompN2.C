...
void SimDataCompN2(int kin)
{
  ...
  if(kin == 2)
  {
    ...
    inel_sim_file = "outfiles/N2_Corr_SIM_GEn_GEN2_He3_dxdy.root";
    //"outfiles/parsed_SIM_IN_GEn_GEN2_He3_dxdy.root";
    pol_func_file = "outfiles/parsed_GEn_pass2_GEN2_simulation.root";
    //N2_dilution_file = "outfiles/N2_Corr_SIM_GEn_GEN2_He3_dxdy.root";
    output_file = "outfiles/AnalysisResults_N2_GEN2.root";
    title_words = "GEN2";
    dx_min_d = -3.0;
    dx_min_i = -3;
    dx_max_d = 2.0;
  ...
  TFile *N2File = TFile::Open(inel_sim_file);
  ...
  TH1D* h_simIN_dx = (TH1D*)hN2dilution_p->Clone("h_simIN_dx");
  ...
  c6->Divide(1,2);
  c6->cd(1);
  hAsym_nwin->Fit("pol0", "Q");
  //TF1 *f0 = hAsym_nwin->GetFunction("pol0");
  //f0->Draw("lsames");
  gStyle->SetOptFit(11);
  gPad->Update();
  hAsym_nwin->Draw();
