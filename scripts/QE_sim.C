...
void QE_sim(const char *kinematic)
{
  ...
  TString inputfile = Form("/volatile/halla/sbs/ktevans/2026SIM/kte_QE_sim_%s_sbs100p_nucleon_np_model2_elastic.root",kinematic);
  ...
  for (size_t iev = 0; iev < T->GetEntries(); iev++)
  {
    ...
    if(e_kine_W2<1.6 && e_kine_W2>0.4 && bb_ps_e>0.2 && abs(((bb_ps_e+bb_sh_e)/bb_tr_p)-1.0)<0.3 && abs(bb_tr_vz)<0.27 && sbs_hcal_e>0.025 && abs(dy_hcal)<0.88)
    {
